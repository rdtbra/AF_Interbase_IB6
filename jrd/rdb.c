...
if (RDB$ATTACH_DATABASE ||
    ...
    {
    RDB_CALL (RDB$ATTACH_DATABASE) (user_status, file, handle, dpb_length, dpb);
    if (stat & 1)
	{
...
stat = gds__attach_database (status_vector, dsc_length (file), 
    file->dsc$a_pointer, handle, len, dpb, db_type);
...

int rdb$cancel_segmented_string (
    int		*user_status,
    BLB		*blob_handle)
{
/**************************************
...

int rdb$close_segmented_string (
    int		*user_status,
    BLB		*blob_handle)
{
/**************************************
...
if ((*tra_handle)->implementation == impl_gds)
    ...
    {
    RDB_CALL (RDB$COMMIT_TRANSACTION) (user_status, &(*tra_handle)->handle);
    }
...
if ((*db_handle)->implementation == impl_gds)
    ...
    {
    RDB_CALL (RDB$COMPILE_REQUEST) (user_status, &database->handle, req_handle,
	blr_length, blr);
    }

if (stat & 1)
    {
    *req_handle = request = allocate_handle (database->implementation, *req_handle);
...
if (db_type = DBTYPE_ISC)
    ...
    stat = gds__create_database (status_vector, dsc_length (file), 
		file->dsc$a_pointer, handle, dpb_length, dpb, db_type);
    if (stat & 1)
	*handle = allocate_handle (impl_gds, *handle);
    return MAP_status_to_rdb (status_vector, user_status);
    }
...
if ((*db_handle)->implementation == impl_gds)
    ...
    {
    RDB_CALL (RDB$CREATE_SEGMENTED_STRING) (user_status, &(*db_handle)->handle,
	&(*tra_handle)->handle, blob_handle, blob_id);
    }

if (stat & 1)
    *blob_handle = allocate_handle ((*db_handle)->implementation, *blob_handle);
...
if ((*db_handle)->implementation == impl_gds)
    ...
    {
    RDB_CALL (RDB$OPEN_SEGMENTED_STRING) (user_status, &(*db_handle)->handle,
	&(*tra_handle)->handle, blob_handle, blob_id);
    }
...
if ((*tra_handle)->implementation == impl_gds)
    ...
    {
    RDB_CALL (RDB$PREPARE_TRANSACTION) (user_status, &(*tra_handle)->handle);
    }
...
if ((*tra_handle)->implementation == impl_gds)
    ...
    {
    RDB_CALL (RDB$ROLLBACK_TRANSACTION) (user_status, &(*tra_handle)->handle);
    }
...
if ((*req_handle)->implementation == impl_gds)
    ...
    {
    RDB_CALL (RDB$UNWIND_REQUEST) (user_status, &(*req_handle)->handle, level);
    }
