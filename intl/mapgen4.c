...
void print_condensed_indexed_table (char *name, TABLE	*table, unsigned short replacement);
void print_indexed_table (char *name, TABLE *table, unsigned short replacement);
...
if (usage)
    {
    ib_fprintf (ib_stderr, "usage: mapgen4 [-condense | -euc | -sjis] <name of unicode map file>\n");
    exit (1);
    }
...
void declare (unsigned short codepoint, 
	unsigned short unicode, char *name)
{
	...
	if (unicode != codepoint_conversion.table [codepoint].equivilant)
	    ib_fprintf (ib_stderr, "Error: duplicate unequal mappings for 0x%04x : 0x%04x and 0x%04x\n",
		codepoint, unicode, codepoint_conversion.table [codepoint].equivilant);
	else
	    ib_fprintf (ib_stderr, "Warning: duplicate entries for codepoint 0x%04x to 0x%04x\n",
		codepoint, unicode);
	}
    else
	...
	if (!unicode_conversion.table [unicode].exists++)
	    ...
	    {
	    if (unicode != UNICODE_REPLACEMENT_CHARACTER)
		ib_fprintf (ib_stderr, "Warning: Multiple mappings to Unicode 0x%04x : 0x%04x and 0x%04x\n",
		    unicode, codepoint, unicode_conversion.table [unicode].equivilant);
	    }
	};
}
...
for (i = 0; i < 256; i++)
    {
    if (table->table [i].exists != 1 ||
        table->table [i].not_defined)
	ib_printf ("/* %02X */     UNICODE_REPLACEMENT_CHARACTER,\n", i); 
    else
	ib_printf ("/* %02X */  0x%04X /* %45s */,\n", 
		i, table->table[i].equivilant,
		table->table [i].name);
    }
ib_printf ("};\n");
}
...
for (index = 0; index < 256; index++)
	...
	ib_printf ("/* U+XX%02X */\tUNICODE_REPLACEMENT_CHARACTER,\t/* %d */\n", index%256, index);
    else
...
while (codepoint < 0xFFFF+1)
	...
	{
	if (replacement == UNICODE_REPLACEMENT_CHARACTER)
	    ib_printf ("/* U+%04X */\tUNICODE_REPLACEMENT_CHARACTER,\t/* %d */\n", 
